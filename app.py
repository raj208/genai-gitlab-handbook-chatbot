...
def _render_sources(sources: list[Source], confidence: str = "high", top_score: float = 0.0) -> None:
    if confidence == "low_confidence":
        st.warning(
            f"Low retrieval confidence (similarity {top_score:.2f}). "
            "The answer may not directly address your question — please verify against the sources."
        )
    if not sources:
        return
...
def main() -> None:
    ...
    st.warning(
        "⚠️ Unofficial assistant. Answers are generated by an LLM from publicly "
        "available GitLab content and may contain inaccuracies. Always verify "
        "against the cited sources before acting on any information."
    )
